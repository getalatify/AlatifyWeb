...
export default function ToolsHubPage({ categoryFilter }: { categoryFilter?: "image" | "document" } = {}) {
  ...
  const introText = React.useMemo(() => {
    ...
    if (categoryFilter === "document") return t("tools-document.intro");
    return t("toolsPage.intro");
  }, [categoryFilter, t]);

  const badgeText = React.useMemo(() => {
    if (categoryFilter === "image") return "Image Tools";
    if (categoryFilter === "document") return "Document Tools";
    return "All Tools";
  }, [categoryFilter]);
  ...
  return (
    <main className="relative flex min-h-screen flex-col items-center p-6 bg-background text-foreground transition-colors duration-300 select-none overflow-x-clip">
      ...
      >
        ...
        {(!categoryFilter || categoryFilter === "image") && (
